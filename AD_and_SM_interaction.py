...
def barplots_intent_vs_targets(df: pd.DataFrame):
    ...
    for col in target_columns:
        ...
        plt.title(f"{col} vs Purchase Intent")
...
def barplots_discount_vs_targets(df: pd.DataFrame):
    ...
    for col in target_columns:
        ...
        plt.title(f"{col} vs Discount Used")
...
def plot_return_share_by_age_group(df: pd.DataFrame):
    ...
    plt.title("📦 Rückgabeanteil nach Altersgruppe")
    plt.xlabel("Altersgruppe")
...
def plot_purchase_frequency_by_age(df: pd.DataFrame):
    ...
    plt.title("🛒 Ø Kauffrequenz nach Altersgruppe")
    plt.xlabel("Altersgruppe")
...
def plot_purchase_amount_by_age(df: pd.DataFrame):
    ...
    plt.title("💰 Ø Kaufbetrag nach Altersgruppe")
    plt.xlabel("Altersgruppe")
...
def plot_total_revenue_by_age_group(df: pd.DataFrame):
    ...
    plt.title("💰 Gesamtumsatz nach Altersgruppe")
    plt.xlabel("Altersgruppe")
...
def heatmap_return_rate_by_channel_and_category(df: pd.DataFrame):
    df = get_ads_users(df)
    ...
    print("\n📊 Rückgabequote pro Kaufkanal (gesamt, in %):")
...
def heatmap_return_by_category_and_INCOME_per_age_group(df: pd.DataFrame):
    df = get_ads_users(df)
    df = categorize_age(df)
    ...
    for group in age_groups:
        ...
        plt.figure(figsize=(10, 6))
        sns.heatmap(pivot, annot=True, fmt=".2f", cmap="coolwarm", linewidths=0.5)
        plt.title(f"📦 Rückgabequote nach Produkt & Income_Level – Altersgruppe {group}")
        plt.xlabel("Income_Level (1 = klein, 3 = hoch)")
        plt.ylabel("Produktkategorie")
        plt.tight_layout()
        plt.show()


def heatmap_return_by_category_and_EDUCATION_per_age_group(df: pd.DataFrame):
    df = get_ads_users(df)
    df = categorize_age(df)
    ...
    for group in age_groups:
        ...
        plt.figure(figsize=(10, 6))
        sns.heatmap(pivot, annot=True, fmt=".2f", cmap="coolwarm", linewidths=0.5)
        plt.title(f"📦 Rückgabequote nach Produkt & Education_Level – Altersgruppe {group}")
        plt.xlabel("education")
        plt.ylabel("Produktkategorie")
        plt.tight_layout()
        plt.show()
        
def heatmap_return_by_category_and_GENDER_per_age_group(df: pd.DataFrame):
    df = get_ads_users(df)
    df = categorize_age(df)
    ...
    for group in age_groups:
        ...
        plt.figure(figsize=(10, 6))
        sns.heatmap(pivot, annot=True, fmt=".2f", cmap="coolwarm", linewidths=0.5)
        plt.title(f"📦 Rückgabequote nach Produkt & Gender – Altersgruppe {group}")
        plt.xlabel("gender")
        plt.ylabel("Produktkategorie")
        plt.tight_layout()
        plt.show()